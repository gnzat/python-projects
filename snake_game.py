...
head.shape("square")
head.color("green")
head.penup() #turtle picks up pen so it does not draw a line when it moves
head.goto(0,0)
head.direction = "stop"
#snake food
food = turtle.Turtle()
...
def move():
    if head.direction == "up":
        y = head.ycor() #used to return the turtle's y coordinate of the current position of turtle
        head.sety(y + 20) #used to set the turtle’s second coordinate to y, leaving the first coordinate unchanged.
    if head.direction == "down":
        y =head.ycor()
        head.sety(y-20)
    if head.direction == "left":
...
#check for collision with border
    if head.xcor()>290 or head.xcor()<-290 or head.ycor()>290 or head.ycor()<-290:
        time.sleep(1) # 1 is the number of seconds execution to be suspended.
        head.goto(0,0)
        head.direction = "stop"
#hide the segments
